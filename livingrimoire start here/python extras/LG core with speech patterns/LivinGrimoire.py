...
class Skill:
    ...
    def set_skill_lobe(self, skill_lobe: int):
        # 1:logical, 2:hardware, 3:ear, 4:skin, 5:eye Chobits, 6:speech patterns
        if 1 <= skill_lobe <= 6:
...
class Brain:
    ...
    def __init__(self):
        ...
        #130326 upgrade
        self.evo_chain: Chobits = Chobits() # speech patterns, translations, speech and action filters
        self.evo_chain_ref:EvoChain = EvoChain()
        self.evo_chain.add_skill(self.evo_chain_ref)
        Brain.imprintSoul(self.logicChobit.getKokoro(), self.hardwareChobit,self.ear,self.skin,self.eye,self.evo_chain)
    ...
    def doIt(self, ear: str, skin: str, eye: str):
        ...
        self.evo_chain.think(ear, skin, eye)
        self._emotion = self.logicChobit.getSoulEmotion()
        temp = self.evo_chain.think(self._logicChobitOutput, skin, eye)
        self.hardwareChobit.think(temp, skin, eye)
    ...
    def add_skill(self, skill: Skill):
        ...
        match skill.get_skill_lobe():
            ...
            case 6:
                if isinstance(skill, EvoChain):
                    self.evo_chain.clear_all_skills()
                    self.evo_chain_ref = skill
                    self.evo_chain.add_skill(skill)
                else:
                    self.evo_chain_ref.add_skill(skill)
    ...
    def remove_skill(self, skill: Skill):
        ...
        match skill.get_skill_lobe():
            ...
            case 6:
                self.evo_chain.clear_all_skills()
                self.evo_chain.add_skill(EvoChain())
