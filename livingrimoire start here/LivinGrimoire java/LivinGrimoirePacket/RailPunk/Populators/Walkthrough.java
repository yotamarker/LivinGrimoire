...
public class Walkthrough extends PopulatorFunc {
    private static final Pattern PATTERN1 = Pattern.compile("^(.*?)\\s+walkthrough is");
    private static final Pattern PATTERN2 = Pattern.compile("walkthrough is\\s+(.*?)$");

    public Walkthrough() {
        super();
        this.regex = "walkthrough";
    }
    ...
    public void populate(RailPunk railbot, String str1) {
        ...
        if (m1.find()) {
            ...
            if (m2.find()) {
                railbot.learnKeyValue(param1 + " walkthrough", m2.group(1));
            }
        }
    }
}
